...
| `auto-discovery-automation.yaml` | Trigger: `msh/+/2/json/mqtt/+`. Extracts `from` (decimal) from `payload_json.from`, `sender` (hex) from topic, `region` from topic. For payload string→dict: `{% if payload is mapping %}payload{% else %}payload \| from_json({}){% endif %}`. Device identifiers: `mesh_node_{{ from }}`. Config keys (`bC`, `sK`, `sA`, `sZ`) subscribe to the **mesh topic** directly (same state_topic as measurement sensors) with `{{ p.<key> | default(states('sensor.node_' ~ from ~ '_<key>')) }}` — when config envelope arrives `<key>` is defined → sensor updates; when measurement envelope arrives `<key>` is Undefined → `default()` falls back to sensor's own current state → value persists. |
